...
<script src="<%=HtmlHelper.getStyleUrl(basePath, "mutiselect/multiselectSrc/jquery.multiselect.js")%>" type="text/javascript"></script>
<link href="<%=HtmlHelper.getStyleUrl(basePath, "mutiselect/multiselectSrc/jquery.multiselect.css")%>" rel="stylesheet" type="text/css" />
<script src="<%=HtmlHelper.getScriptUrl(basePath, "common/jquery-ui-timepicker-addon.js")%>" type="text/javascript"></script>
<link href="<%=HtmlHelper.getStyleUrl(basePath, "common/jquery-datetimepicker.css")%>" rel="stylesheet" type="text/css" />
<style type="text/css">
...
<script type="text/javascript">
	...
	function initTimeControl() {
		...
		$('#beginTime').datetimepicker({
			...
			onSelect : function(selectedDateTime) {
				...
				$('#activityTime').datetimepicker('option', 'minDate', 
						$('#beginTime').datetimepicker('getDate'));
			}
		});
	...
	function verifyEverything(vo){
		...
		if (activityName == null || "" == activityName) {
			...
			ret['isOk'] = false;
			ret['info'] = "请输入最少人数";
		} else if (isAllSelect == false && count == 0) {
			ret['isOk'] = false;
			ret['info'] = "请选择人员";
		...
		<table class="singlepage_table">
			<tbody>
				...
				<tr>
					...
					<td class="singlepage_right">
						<input type="text" id="activityTime" name="activityTime" />
				...
				<tr>
					...
					<td class="singlepage_right">
						...
						<input type="checkbox" id="isAllSelect" name="isAllSelect" onchange="isAllSelect()"
							value="<%=RmConstant.Activity.ALLSELECT_TRUE%>" />
						<span>全选</span>
					</td>
				</tr>
				<tr>
					...
					<td class="singlepage_right">
						<input type="text" id="minimum" name="minimum" onkeypress="return isNum(event)"
							onblur="justAllowNumber(this)" style="width: 75px;" />
						<span style="margin: 0px 2px;">至</span>
						<input type="text" id="maximum" name="maximum" onkeypress="return isNum(event)"
							onblur="justAllowNumber(this)" style="width: 75px;" />
					</td>
				</tr>
				<tr>
